...
export const questionRuleInputSchema = z.object({
  title: z
    .string()
    .trim()
    .min(4, "기준 이름을 4자 이상 입력해 주세요.")
    .max(120),
  school: z
    .string()
    .trim()
    .min(2, "학교 또는 공통 범위를 입력해 주세요.")
    .max(80),
...
export const questionRulePatchSchema = questionRuleInputSchema
  .partial()
  .refine((value) => Object.keys(value).length > 0, "변경할 값이 필요합니다.");

export const questionRuleSchema = questionRuleInputSchema.extend({
  id: z.string().min(1),
  updatedAt: z.string().datetime(),
});

export const questionRulesResponseSchema = z.object({
  rules: z.array(questionRuleSchema),
});

...
const tours: Record<UserRole, TourStep[]> = {
  user: [
    {
      selector: "[data-tour='student-next-action']",
      title: "오늘은 여기부터 시작하세요",
      detail:
        "AIHOW가 현재 준비 상태에서 가장 영향이 큰 한 가지를 먼저 제안합니다.",
    },
    {
      selector: "[data-tour='student-admissions']",
      title: "지원 정보는 이곳에서 확인해요",
      detail: "공식 원문과 검토가 끝난 정보만 준비 과정에 반영됩니다.",
    },
    {
      selector: "[data-tour='student-menu']",
      title: "준비 단계는 아래에서 이어가세요",
      detail:
        "자소서부터 질문 연습, 모의면접, 파이널 노트까지 순서대로 이어집니다.",
    },
  ],
  admin: [
    {
    ...
    {
      selector: "[data-tour='admin-prompt-recipes']",
      title: "지금 적용 중인 코칭 레시피입니다",
      detail:
        "학생 수가 늘어도 이 기준을 한 번 관리하면 모든 관련 결과에 일관되게 반영됩니다.",
    },
    {
      selector: "[data-tour='admin-menu']",
      title: "필요한 도구만 여기에서 여세요",
      detail:
        "메뉴를 전부 익힐 필요 없이, 지금 설계할 수업 단계에 맞는 화면으로 이동하면 됩니다.",
    },
  ],
};
...
export function FirstVisitTour({
  ...
  return createPortal(
    <div className="viewport-overlay fixed z-[110]" role="presentation">
      ...
      <section
        className="surface fixed overflow-y-auto overscroll-contain bg-[var(--surface)] p-5 shadow-[var(--shadow-md)] [scrollbar-gutter:stable] sm:p-7"
        style={{
          left,
      ...
      >
        <div className="flex items-start gap-3">
          <span className="font-mono text-[10px] font-bold tracking-[.14em] text-[var(--brand)]">
            수업 안내 · {String(index + 1).padStart(2, "0")} /{" "}
        ...
        <h2 className="mt-4 text-xl font-black tracking-[-.04em] sm:text-2xl">
          {step.title}
        </h2>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-2 border-t border-[var(--border)] pt-4">
          ...
          <div className="flex gap-2 text-[10px] font-bold text-[var(--text-tertiary)]">
            <button type="button" onClick={postponeTour}>
              24시간 숨기기
            </button>
            <button type="button" onClick={neverTour}>
              다시 보지 않기
            </button>
          </div>
        </div>

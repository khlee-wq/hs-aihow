...
export function StudentDashboard({
  ...
  const [admissionsMapOpen, setAdmissionsMapOpen] = useState(false);
  const [admissionsMapView, setAdmissionsMapView] = useState<"flow" | "guide">(
    "flow",
  );
  ...
  return (
    <>
      <section
        className="student-dashboard-canvas practice-session-canvas liquid-glass-group relative overflow-hidden rounded-[2rem] px-4 py-6 sm:px-7 sm:py-7 lg:px-8 lg:py-6"
        data-testid="student-dashboard"
        aria-busy={isLoading}
      >
        <div className="learning-canvas-content float-in relative z-10 space-y-6 md:space-y-7">
          <header className="border-b border-[var(--border)] pb-5 text-center">
            {isLoading ? (
              <DashboardHeaderSkeleton />
            ) : (
              <LearningIntroHero
          ...
          >
            <div className="bg-[var(--brand-soft)] p-5 sm:p-6" data-motion-item>
              ...
              <p className="mt-2 max-w-xl text-sm leading-6 text-[var(--text-secondary)]">
                자소서 근거를 확인하고, 면접에서 먼저 준비할 포인트와 예상
                질문을 받아보세요.
              </p>
              <Link
            ...
            <div className="bg-[var(--surface)] p-5 sm:p-6" data-motion-item>
              <p className="flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[.14em] text-[var(--text-tertiary)]">
                <Infinity className="size-3.5 text-[var(--brand)]" /> Continue
                with subscription
              </p>
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
                구독하면 분석과 질문 훈련, 영상 가이드를 반복해서 활용하며 면접
                직전 노트까지 한 흐름으로 관리합니다.
              </p>
              <button
                type="button"
          ...
          >
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,.85fr)]">
              <div data-motion-item>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
                  공개된 자료는 그대로 쌓지 않습니다. 공식 원문을 확인하고,
                  소장님의 검수를 거쳐 지원 기준·예상 질문·영상 가이드로
                  연결합니다.
                </p>
              </div>
              <div
              ...
              >
                <div className="p-4">
                  ...
                  <p className="mt-2 text-lg font-bold tracking-[-.04em]">
                    2027
                  </p>
                </div>
                <div className="p-4">
            ...
            >
              <p className="flex items-center gap-1.5 text-[11px] leading-5 text-[var(--text-tertiary)]">
                <FileClock className="size-3.5 shrink-0" /> 공개 전 자료는
                확정된 지원 조건으로 안내하지 않습니다.
              </p>
              <button
                type="button"
